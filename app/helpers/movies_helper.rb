module MoviesHelper
  ...
  def tiny_movie_poster(movie)
    image_tag movie.poster.url(:tiny), class: 'tiny-poster', width: 22, height: 30
  end
  ...
  def tiny_movie_poster_link(movie, job = nil)
    ...
    link_to tiny_movie_poster(movie), movie.poster.url(:original), class: 'tiny-poster-link', rel: rel, title: title, data: { movie_url: movie_path(movie) }
  end
end
